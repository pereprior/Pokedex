...
@Composable
fun TablaDeTipos(vm: PokemonViewModel, drawerState: DrawerState, navController: NavHostController) {
    LaunchedEffect(Unit) {
        vm.getTypeList()
    }
    ...
    if (typeList.isEmpty()) {
        ...
        Scaffold(
            topBar = { MyTopAppBar(drawerState, navController, typeList) },
            content = {
                LazyColumn(modifier = Modifier.fillMaxSize()) {
                    items(typeList) { type ->
                        Column(
                            ...
                            Button(
                                onClick = {
                                    navController.navigate("TypeView/${type.name}")
                                },
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .height(55.dp),
